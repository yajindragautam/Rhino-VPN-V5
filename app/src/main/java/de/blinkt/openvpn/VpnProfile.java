...
//    public static final long MAX_EMBED_FILE_SIZE = 2048 * 1024; // 2048kB
    ...
    public String getConfigFile(Context context, boolean configForOvpn3) {
        ...
        if (mAuthenticationType != TYPE_STATICKEYS) {
            if (mCheckRemoteCN) {
                if (mRemoteCN == null || mRemoteCN.isEmpty())
    ...
    private String cidrToIPAndNetmask(String route) {
        ...
        String netmask = String.format(Locale.ENGLISH, "%d.%d.%d.%d", (nm & 0xff000000L) >> 24, (nm & 0xff0000) >> 16, (nm & 0xff00) >> 8, nm & 0xff);
    ...
    synchronized String[] getExternalCertificates(Context context, int tries) {
        ...
        try {
            ...
            return new String[]{ca, extra, user};
                 ...
                 IllegalArgumentException | CertificateException e) {
            Utils.getErrors(e);
    ...
    public String getPasswordPrivateKey() {
        ...
        return switch (mAuthenticationType) {
            case TYPE_PKCS12, TYPE_USERPASS_PKCS12 -> mPKCS12Password;
            case TYPE_CERTIFICATES, TYPE_USERPASS_CERTIFICATES -> mKeyPassword;
            default -> null;
        };
    }
    ...
    public boolean isUserPWAuth() {
        return switch (mAuthenticationType) {
            case TYPE_USERPASS, TYPE_USERPASS_CERTIFICATES, TYPE_USERPASS_KEYSTORE,
                 TYPE_USERPASS_PKCS12 -> true;
            default -> false;
        };
    }
    ...
    public int needUserPWInput(String transientCertOrPkcs12PW, String mTransientAuthPW) {
        if ((mAuthenticationType == TYPE_PKCS12 || mAuthenticationType == TYPE_USERPASS_PKCS12) &&
                (mPKCS12Password == null || mPKCS12Password.isEmpty())) {
    ...
    private byte[] getExtAppSignedData(Context c, byte[] data, OpenVPNManagement.SignaturePadding padding, String saltlen, String hashalg, boolean needDigest) {
        ...
        Bundle extra = getExtra(padding, saltlen, hashalg);
    ...
    @NonNull
    private static Bundle getExtra(OpenVPNManagement.SignaturePadding padding, String saltlen, String hashalg) {
        Bundle extra = new Bundle();
        RsaPaddingType paddingType = switch (padding) {
            case RSA_PKCS1_PADDING -> RsaPaddingType.PKCS1_PADDING;
            case RSA_PKCS1_PSS_PADDING -> RsaPaddingType.RSAPSS_PADDING;
            default -> RsaPaddingType.NO_PADDING;
        };
        ...
        try {
            ...
            if (needDigest || keyalgorithm.equals("EC")) {
             ...
             /* ECB is perfectly fine in this special case, since we are using it for
                ...
                Cipher signer = switch (padding) {
                    case RSA_PKCS1_PADDING -> Cipher.getInstance("RSA/ECB/PKCS1PADDING");
                    case NO_PADDING -> Cipher.getInstance("RSA/ECB/NoPadding");
    ...
    private int getHashtype(String digest) throws NoSuchAlgorithmException {
        return switch (digest) {
            ...
            case "SHA384" -> 4;
            case "SHA512" -> 5;
            default -> throw new NoSuchAlgorithmException("Unknown digest algorithm: " + digest);
        };
    }
    ...
    private byte[] doDigestSign(PrivateKey privkey, byte[] data, OpenVPNManagement.SignaturePadding padding, String hashalg, String saltlen) throws SignatureException, NoSuchAlgorithmException, InvalidAlgorithmParameterException, InvalidKeyException {
        ...
        if (privkey.getAlgorithm().equals("EC")) {
            ...
            PSSParameterSpec pssspec = switch (hashalg) {
                case "SHA256" ->
