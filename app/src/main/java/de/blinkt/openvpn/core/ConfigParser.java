...
/** @noinspection StringConcatenationInLoop*/ // And remember, this is valid :)
// --<foo>
// bar
// </foo>
public class ConfigParser {
    ...
    public VpnProfile convertProfile() throws ConfigParseError, IOException {
        ...
        if (routes != null) {
            ...
            StringBuilder routeExcluded = getRouteExcluded(routes, routeopt);
        ...
        if (verifyx509name != null) {
            ...
            if (verifyx509name.size() > 2) {
                switch (verifyx509name.get(2)) {
                    case "name" -> np.mX509AuthType = VpnProfile.X509_VERIFY_TLSREMOTE_RDN;
                    case "subject" -> np.mX509AuthType = VpnProfile.X509_VERIFY_TLSREMOTE_DN;
                    case "name-prefix" ->
        ...
        final Vector<String> x509usernamefield = getOption("x509-username-field", 1, 1);
        if (x509usernamefield != null) {
            np.mx509UsernameField = x509usernamefield.get(1);
        }

        final Vector<String> verb = getOption("verb", 1, 1);
        if (verb != null) {
            np.mVerb = verb.get(1);
        }
        ...
        if (authretry != null) {
            switch (authretry.get(1)) {
                case "none" -> np.mAuthRetry = VpnProfile.AUTH_RETRY_NONE_FORGET;
                case "nointeract", "interact" -> np.mAuthRetry = VpnProfile.AUTH_RETRY_NOINTERACT;
                default ->
    ...
    @NonNull
    private static StringBuilder getRouteExcluded(Vector<Vector<String>> routes, StringBuilder routeopt) throws ConfigParseError {
    ...
    private Pair<Connection, Connection[]> parseConnectionOptions(Connection connDefault) throws ConfigParseError {
        ...
        if (!(conn.mCustomConfiguration == null || conn.mCustomConfiguration.trim().isEmpty()))
            ...
            for (Vector<String> redirect : defgw)
                for (int i = 1; i < redirect.size(); i++) {
                    switch (redirect.get(i)) {
                        case "block-local" -> np.mAllowLocalLAN = false;
                        case "unblock-local" -> np.mAllowLocalLAN = true;
                        case "!ipv4" -> noIpv4 = true;
                        case "ipv6" -> np.mUseDefaultRoutev6 = true;
                    }
                }
