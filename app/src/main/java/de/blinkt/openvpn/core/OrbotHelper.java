...
public class OrbotHelper {
    ...
    public synchronized void addStatusCallback(Context c, StatusCallback cb) {
        if (statusCallbacks.isEmpty()) {
            IntentFilter filter = new IntentFilter(OrbotHelper.ACTION_STATUS);
            mContext = c.getApplicationContext();
            ContextCompat.registerReceiver(mContext, orbotStatusReceiver, filter, ContextCompat.RECEIVER_NOT_EXPORTED);
        }
